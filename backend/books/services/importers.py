...
class BookImport:
    ...
    def save_from_search(self, docs: list[dict] | None) -> list[Book] | None:
        return (
            [self._upsert_book(doc) for doc in docs if self._is_valid(doc)]
            if docs
            else None
        )
    ...
    def _upsert_book(self, data: dict) -> Book:
        ...
        subjects = SubjectImport()._save_many(data["subject"])
        authors = AuthorImport()._save_many(
...
class AuthorImport:
    ...
    def save_authors_from_search(self, docs: list[dict] | None) -> list[Author] | None:
        return (
            [self._upsert_author(doc) for doc in docs if self._is_valid_author(doc)]
            if docs
            else None
        )
    ...
    def save_from_works(self, entries: list[dict] | None) -> list[Book] | None:
        ...
        return (
            [
                self._upsert_author_book(doc)
                for doc in entries
                if self._is_valid_author_work(doc)
            ]
            if entries
            else None
        )
    ...
    def _upsert_author_book(self, data: dict) -> Book:
        defaults = BookImport().get_detail_information(data)

        if defaults["cover_ids"] and len(defaults["cover_ids"]) >= 1:
            defaults["cover_i"] = defaults["cover_ids"][0]

        subjects = SubjectImport()._save_many(data["subjects"])
        authors = self._save_many_keys(data["authors"])

        book, _ = Book.objects.update_or_create(
            openlibrary_key=_clear_key(data["key"]), defaults=defaults
        )
    ...
    @staticmethod
    def _save_many(names: list[str], keys: list[str]) -> list[Author]:
        result = []

        for key, name in zip(keys, names):
            author, _ = Author.objects.update_or_create(
                openlibrary_key=key, defaults={"name": name}
            )
    ...
    @staticmethod
    def _save_many_keys(keys: list[dict] | None):
        result = []
        if keys:
            for item in keys:
                author_data = item.get("author")

                if author_data and "key" in author_data:
                    author = Author.objects.filter(
                        openlibrary_key=_clear_key(author_data["key"])
                    ).first()
    ...
    def _is_valid_author(data: dict) -> bool:
        requeired_keys = ["birth_date", "key", "name"]

        return all(data.get(key) for key in requeired_keys)
...
class SubjectImport:
    ...
    def _save_many(self, raw_subjects: list[str]) -> list[Subject]:
        result = []
...
def _clean_covers(covers: list) -> list:
    return [
        cover for cover in covers if covers and isinstance(cover, int) and cover > 0
    ]
